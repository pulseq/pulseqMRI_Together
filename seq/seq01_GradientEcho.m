...
TE=4e-3;                        % TE
%TE=[4 9 15]*1e-3;               % optionally give a vector here to have multiple TEs (e.g. for field mapping)
% TODO: change to multiple contrasts, change order of loops...
...
delayTR=ceil((TR - mr.calcDuration(gz) - max(mr.calcDuration(gxPre,gzReph),delayTE) ...
...
for i=1:Ny % loop over phase encodes
    for c=1:length(TE) % loop over TEs
        ...
        %seq.addBlock(mr.makeDelay(delayTE(c)),gxPre,gyPre,gzReph);
        seq.addBlock(mr.align('left', mr.makeDelay(delayTE(c)),gyPre,gzReph,'right',gxPre)); % stitch the read prephase to the readout gradient 
        seq.addBlock(gx,adc);
